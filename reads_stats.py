...
for n,d in enumerate(id_names):
    ...
    # Coverage
    cov_filename = os.path.join(fileid,str("%s.depth.txt" % fileid))
    ...
    # Read lengths
    rlen_filename = os.path.join(fileid,str("%s.readlengths.txt" % fileid))
...
#%%######################### Plotting #################################
pdf = matplotlib.backends.backend_pdf.PdfPages("reads_stats.pdf")
# Colors
