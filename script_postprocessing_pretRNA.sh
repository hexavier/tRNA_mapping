...
$segemehl --evalue 500 --differences 3 --maxinterval 1000 --accuracy 85 --index ${genomeDir}/${tRNAName}_cluster_pre-tRNA.idx --database ${genomeDir}/${tRNAName}_cluster_pre-tRNA.fa --nomatchfilename ${bn}_unmatched.fastq --query $n | $samtools view -bS | $samtools sort -T ${bn} -o ${bn}.bam
...
$gatk -T PrintReads -R ${genomeDir}/${tRNAName}_cluster_pre-tRNA.fa -I ${bn}.mod.bam -o ${bn}.temp.bam -rf UnmappedRead -rf ReassignMappingQuality --default_mapping_quality 60
...
##realignment
$gatk -T RealignerTargetCreator -R ${genomeDir}/${tRNAName}_cluster_pre-tRNA.fa -I ${bn}.mod.bam -o ${bn}.temp.intervals
$gatk -T IndelRealigner -R ${genomeDir}/${tRNAName}_cluster_pre-tRNA.fa -I ${bn}.mod.bam -targetIntervals ${bn}.temp.intervals -o ${bn}.realigned.bam --maxReadsForRealignment 500000
...
$gatk -R ${genomeDir}/${tRNAName}_cluster_pre-tRNA.fa -T UnifiedGenotyper -I ${bn}.mmHandled.bam -o ${bn}.GATK.vcf -stand_call_conf 50.0
grep -i -v lowqual ${bn}.GATK.vcf > ${bn}.GATK_filtered.vcf
    
## Allele-specific expression analysis
$samtools idxstats ${bn}.mmHandled.bam > ${bn}.expression.txt
$gatk -R ${genomeDir}/${tRNAName}_cluster_pre-tRNA.fa -T ASEReadCounter -I ${bn}.mmHandled.bam -o ${bn}.ASE.csv -sites ${bn}.GATK_filtered.vcf

## Compute expression
python ${scriptDir}/compute_expression.py ${bn}.expression.txt ${bn}.ASE.csv ${genomeDir}/${tRNAName}_clusterInfo_pre-tRNA.fa ${genomeDir}/tRNAs.txt ${bn}.RPM.csv
