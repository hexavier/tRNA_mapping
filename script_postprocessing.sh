...
if [ "$multimapperHandling" == "uniq" ]; then
  ...
  # indexing quant file
  $samtools index ${bn}.mmHandled.quant.bam
  $picard BuildBamIndex I=${bn}.mmHandled.quant.bam O=${bn}.mmHandled.quant.bai
  # keep also file with unique reads for modification calling
  $samtools view -h ${bn}.realigned.bam | grep -P 'NH:i:1\D'\|'^@' | $samtools view -bS | $samtools sort -T ${bn} -o ${bn}.mmHandled.bam
